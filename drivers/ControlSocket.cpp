...
t_command const& CControlSocket::MapCommand(CStdString const& command, CStdString const& args)
{
	...
	if( it != command_map.end() ) {
		...
		if( it->second.bHasargs && args.empty() ) {
			...
			if (!m_RecvLineBuffer.empty()) {
				m_owner.PostThreadMessage(WM_NETCONNECTOR_THREADMSG, FTM_COMMAND, m_userid);
			}
		}
...
void CControlSocket::ParseCommand()
{
	...
	case commands::SYST:
		Send(_T("215 UNIX emulated by NETCONNECTOR"));
		break;
	case commands::NOOP:
	case commands::NOP:
		Send(_T("200 OK"));
	...
	case commands::HELP:
		if (args.empty()) {
			...
			if( it != command_map.end() ) {
				CStdString str;
				str.Format(_T("214 Command %s is supported by NETCONNECTOR Server"), args);
				Send(str);
				break;
			}
			else {
				CStdString str;
				str.Format(_T("502 Command %s is not recognized or supported by NETCONNECTOR Server"), args);
				Send(str);
			}
		}
		break;
